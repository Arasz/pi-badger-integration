...
- Tick seam contract (PKG-2→PKG-3 interface, fixed before PKG-3 starts): `tickCoordinator(store: BusStore, sessions: RegistrySnapshot, opts: { budget: number; now: number; disabled: boolean })` where `RegistrySnapshot = { entries: Array<{ sessionId: string; projectId: string | null; lastSeenMs: number }>; version: string }` produced by PKG-1's registry reader; `version` = `max(lastSeenMs)+count` snapshot hash per tick (no persistent version anywhere — in-memory counters die with the process on a multi-process bus).
