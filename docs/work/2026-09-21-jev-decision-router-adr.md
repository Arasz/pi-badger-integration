...
| R10 | Skip chain (in order): kill switches → session off → missing key → cooldown → slash-prefixed → marker-prefixed (provisional: the predicate defaults to never until wired — plan F18) → short prompt (< 12 non-whitespace chars) → in-flight → cache hit. Queued follow-ups do not refire the hook. |
