...
**A4 — MG-1 mechanism verified from the installed pi source (2026-09-23).** The progress chain is event-loop-scheduled, so it paints while the blocking hook awaits: `ctx.ui.setStatus` -> `interactive-mode.setExtensionStatus` (`dist/modes/interactive/interactive-mode.js:1661-1663`) -> `footerDataProvider.setExtensionStatus` + `ui.requestRender()` -> `TUI.requestRender()` -> `process.nextTick(() => scheduleRender())` (`@earendil-works/pi-tui/dist/tui.js:499-508`). The hook awaits network I/O, so the event loop is free to run that callback and repaint; the only failure mode would be a synchronously-blocking hook, which this one is not. Status: `[READ]` mechanism verified; live pixel confirmation still recorded as the MG-1 manual step.
